...
#define IStoSample(s)	((Limit((s) >> (FM_ISHIFT+3), 0xffff, -0x10000) * fmvolume) >> 14)
    ...
    // @TODO: Здесь я изменил код, чтобы запись была в МОНО буфер, а не СТЕРЕО
    // Раскомментируйте StoreSample(...), если хотите всё вернуть
	if (actch & 0x15)
	{
		if (interpolation)
		{
...
bool outSlots[3][4];
uint8 TLevels[4];
void OPN::SetVoiceOpEnvel(uint8 voiceNum, uint8 opNum, uint8 ar, uint8 dr, uint8 sr, uint8 rr, uint8 sl, uint8 tl)
{
	const static uint8 sltable[16] = { 0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 124 };
	Operator &oper = ch[voiceNum].op[opNum];
	oper.SetAR(ar * 2);
	oper.SetDR(dr * 2);
	oper.SetSR(sr * 2);
	oper.SetRR(rr * 4 + 2);
	oper.SetSL(sltable[sl & 15]);
	oper.SetTL(tl, false);

	TLevels[voiceNum * 4 + opNum] = tl;
}
void OPN::SetVoiceOpOther(uint8 voiceNum, uint8 opNum, uint8 ks, uint8 mul, uint8 det, bool amon)
{
	Operator &oper = ch[voiceNum].op[opNum];
	oper.SetKS(ks);
	oper.SetMULTI(mul);
...
void OPN::RememberAlgorithm(uint8 voice, uint8 alg)
{
	const bool algOutputs[8][4] = {
	...
	for (int i = 0; i < 4; i++)
		outSlots[voice][i] = algOutputs[alg][i];
}

void OPN::SetVelocity(uint8 voice, uint8 pmdVelocity)
{
	uint8 tlOffset = pmdVelocity;

	for (int i = 0; i < 4; i++)
	{
		if (outSlots[i])
		{
			uint8 baseTL = TLevels[voice * 4 + i];
			ch[voice].op[i].SetTL(baseTL + tlOffset, false);
		}
	}
}
#endif // BUILD_OPN
